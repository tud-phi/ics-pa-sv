# Intelligent Control Systems (2025): Practical Assignment
...
This framework requires **Python 3.10/3.11/3.12**. For example, Ubuntu 24.04 ships with Python 3.12 and is therefore a suitable choice.
Please note that some required dependencies (for example PyTorch) might not be updated yet to work with Python 3.13 or newer.
...
You can open the Jupyter notebooks in the editor and then use the integrated Jupyter notebook extension to execute them. If you are prompted to select a kernel, please choose the kernel `Python 3.12.x /usr/local/bin/python`.
